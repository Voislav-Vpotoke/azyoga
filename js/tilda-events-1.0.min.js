		...
		function e() {
			...
			try {
				var i = document.getElementById('allrecords'),
					a = i ? i.getAttribute('data-tilda-cookie') : null
				;(i && 'no' === a) || Tilda.saveUTM()
			} catch (e) {}
			...
			function o(e) {
				...
				if (e.target && t) {
					...
					if (!r) {
						var o = t.closest('.r')
						r = '/tilda/click/' + (o ? o.getAttribute('id') : '') + '/?url=' + a
					}
					if (
						(Tilda.sendEventToStatistics(r, i),
						'http' === a.substring(0, 4) && -1 === a.indexOf('.typeform.com/'))
					)
						return (
		...
		;(Tilda.sendEcommerceEvent = function (e, t) {
			...
			for (
				...
				d > '' && (d += ', '),
					...
					void 0 !== a.options &&
						a.options.length > 0 &&
						Array.prototype.forEach.call(a.options, function (e) {
							i += e.option + ': ' + e.variant + '; '
						})
				var p = { name: a.name, price: a.price, variant: i, quantity: 1 }
				a.id && a.id > 0 && ((id = a.id), (p.id = a.id)),
					a.uid && a.uid > 0 && ((m = a.uid), (p.uid = a.uid)),
					a.recid && a.recid > 0 && ((n = a.recid), (p.recid = a.recid)),
					a.lid && a.lid > 0 && ((l = a.lid), (p.lid = a.lid)),
					a.sku && a.sku > 0 && (p.sku = a.sku),
					(c[c.length] = p)
			}
			;('add' !== e && 'remove' !== e) ||
				((r = '/tilda/cart/' + e + '/'),
				...
				((p = { ecommerce: {} }).ecommerce[e] = { products: c }),
				Tilda.sendEventToStatistics(r, d, p, o)
		}),
			(Tilda.sendEventToStatistics = function (e, t, r, i) {
				var a,
					d = '/' === e.substring(0, 1),
					o = []
				;(y = document.getElementById('allrecords')) &&
					(a =
						null !== y.getAttribute('data-fb-event')
							? y.getAttribute('data-fb-event')
				...
				y &&
					(n =
						null !== y.getAttribute('data-vk-event')
							? y.getAttribute('data-vk-event')
				...
				if (
					((m = y.getAttribute('data-tilda-currency')
						? y.getAttribute('data-tilda-currency')
						: u && u.getAttribute('data-project-currency-code')
						? u.getAttribute('data-project-currency-code')
						: 'RUB'),
					r || (r = window.location.href),
					(i = i ? parseFloat(i) : 0) > 0)
				)
					if (
				...
				try {
					if (window.gtagTrackerID && 'gtag' === window.mainTracker)
						if (d)
							if (r && r.event)
								if ('purchase' === r.event) {
									o = r.ecommerce.purchase.products
									for (var s = 0, v = 0; v < o.length; v++)
										if ('delivery' === o[v].id) {
											s = o[v]
											break
										}
							...
							try {
								if (
									(window.Tilda.isLoadGAEcommerce ||
										...
										ga('require', 'ec')),
									ga('set', 'currencyCode', m),
									'purchase' === r.event)
								) {
									var w = r.ecommerce.purchase.products.length
									for (v = 0; v < w; v++)
										(T = r.ecommerce.purchase.products[v]),
											ga('ec:addProduct', {
												id: T.id || v,
												name: T.name,
									...
									for (w = r.ecommerce.add.products.length, v = 0; v < w; v++)
										(T = r.ecommerce.add.products[v]),
											ga('ec:addProduct', {
												id: T.id || v,
												name: T.name,
												price: parseFloat(T.price).toFixed(2),
									...
									for (
										w = r.ecommerce.remove.products.length, v = 0;
										v < w;
										v++
									)
										(T = r.ecommerce.remove.products[v]),
											ga('ec:addProduct', {
												id: T.id || v,
												name: T.name,
												price: parseFloat(T.price).toFixed(2),
									...
									for (
										w = r.ecommerce.detail.products.length, v = 0;
										v < w;
										v++
									)
										(T = r.ecommerce.detail.products[v]),
											ga('ec:addProduct', {
												id: T.id || v,
												name: T.name,
												price: parseFloat(T.price).toFixed(2),
				...
				if (
					...
					if (d) {
						var g = { title: t }
						i > 0
							? ((g.params = { order_price: i }),
							  m && (g.params.currency = m),
							  ym(window.mainMetrikaId, 'hit', e, g))
							: ym(window.mainMetrikaId, 'hit', e, g)
					} else
						i > 0
							? ((g = { order_price: i }),
							  m && (g.currency = m),
							  ym(window.mainMetrikaId, 'reachGoal', e, g))
							...
							if (
								...
								if (r && r.event && i > 0)
									if ('addToCart' === r.event && r.ecommerce.add) {
										...
										for (
											w = r.ecommerce.detail.products.length, f = [], v = 0;
											v < w;
											v++
										)
											(T = r.ecommerce.detail.products[v]),
				...
				if (
					...
					try {
						if (d) {
							var y,
								h = (y = document.getElementById('allrecords'))
									? y.getAttribute('data-vk-price-list-id')
									: null,
								k = (h && parseInt(h)) || 0,
								_ = '',
								b = !1
							if (r && r.event)
								if (
									((b = { products: [], currency_code: '', total_price: 0 }),
									'purchase' === r.event && r.ecommerce.purchase)
								)
									if (i > 0 && k > 0) {
										b.currency_code = m
										for (
											w = r.ecommerce.purchase.products.length, f = [], v = 0;
											v < w;
											v++
										)
											(T = r.ecommerce.purchase.products[v]),
									...
									if (i > 0 && k > 0) {
										b.currency_code = m
										for (
											w = r.ecommerce.add.products.length, f = [], v = 0;
											v < w;
											v++
										)
											(T = r.ecommerce.add.products[v]),
									...
									if (i > 0 && k > 0) {
										...
										(_ = 't-view-product'),
											r.ecommerce.detail[0] &&
												...
												(_ += '-' + r.ecommerce.detail[0].uid)
								else if ('removeFromCart' === r.event && r.ecommerce.remmove)
									if (i > 0 && k > 0) {
										var T
										b.currency_code = m
										for (
											w = r.ecommerce.remove.products.length, f = [], v = 0;
											v < w;
											v++
										)
											(T = r.ecommerce.remove.products[v]),
								...
								_ =
									-1 !== e.indexOf('tilda/') && -1 !== e.indexOf('/payment/')
										...
										: 't-' + e.replace('/', '-')
							k > 0 && b && b.currency_code
								? (VK.Retargeting.Event('purchase'),
								  VK.Retargeting.ProductEvent(k, _, b))
								: (VK.Retargeting.Event(_),
								  't-purchase' === _.substring(0, 10)
										? VK.Goal('purchase')
										: 't-lead' === _.substring(0, 6) && VK.Goal('lead'))
				...
				try {
					...
					if (
						...
						var i,
							a = t.split('&')
						for (var d in a)
							'amp;' === (i = a[d].split('='))[0].substring(0, 4) &&
								(i[0] = i[0].substring(4)),
								'utm_' === i[0].substring(0, 4) && (r = r + a[d] + '|||')
						if (r.length > 0) {
							var o = new Date()
