...
;(window.t_forms__lang = (
	...
	t_forms__onReady(function () {
		...
		if (r || o || a) {
			...
			document.head.querySelector('script[src*="' + n + '"]') ||
				...
				(s.src = 'https://static.tildacdn.com/js/' + n + '.min.js'),
				(s.onerror = function () {
					...
					case 'date':
						f.length > 0 &&
							!f.match(
								...
								}[n.getAttribute('data-tilda-dateformat')] ||
	...
	(window.tildaForm.showErrorInPopup = function (t, e) {
			...
			if (e[u] && e[u].obj) {
				...
				for (var p = 0; p < e[u].type.length; p++) {
					var f = e[u].type[p],
						_ = t_forms__getMsg(f)
					;(l = ''),
						(i = t.querySelector('.js-rule-error-' + f))
							? (i.textContent && i.innerText) || !_ || -1 !== d.indexOf(_)
								? ((l = i.textContent || i.innerText),
								  -1 === d.indexOf(_) &&
										(d = d + '<p class="t-form__errorbox-item">' + l + '</p>'))
								: (d = d + '<p class="t-form__errorbox-item">' + _ + '</p>')
							: _ &&
							  -1 === d.indexOf(_) &&
							  (d = d + '<p class="t-form__errorbox-item">' + _ + '</p>'),
						n &&
							(!l && t_forms__getMsg(f + 'field')
								? (l = t_forms__getMsg(f + 'field'))
								: _ && (l = _),
							l &&
								a &&
		...
		if (
			...
			var h = t.closest('.r'),
				g = 'focus'
			document.addEventListener || (g = 'focusin'),
				t_removeEventListener(h, g, w),
				t_addEventListener(h, g, w, !0),
				t_removeEventListener(h, 'change', w),
				t_addEventListener(h, 'change', w, !0)
		}
		...
		for (var o = '', a = !1, n = '', i = '', s = '', l = 0; l < e.length; l++)
			if (e[l] && e[l].obj) {
				...
				for (d = 0; d < e[l].type.length; d++) {
					var u = e[l].type[d]
					if (
						((s = ''), (n = t.querySelectorAll('.js-rule-error-' + u)).length)
					)
				...
				(xhr.onreadystatechange = function () {
					if (4 === xhr.readyState)
						if (xhr.status >= 200 && xhr.status < 400) {
								...
								if ('{' == d.substring(0, 1)) {
									...
									if ('object' == typeof o)
										if (o && o.message && 'OK' !== o.message)
											...
											var a = (l = t_forms__getErrorContainers(
													form,
													'Unknown error. Please, try again later.'
												)).errorBoxes,
												n = l.allError
							...
							window.Tilda &&
							'function' == typeof window.Tilda.sendEventToStatistics
								? window.Tilda.sendEventToStatistics(i, s, '', 0)
								: 'undefined' != typeof ga && 'tilda' !== window.mainTracker
								? ga('send', { hitType: 'pageview', page: i, title: s })
								: window.mainMetrika > '' &&
								  window[window.mainMetrika] &&
							...
							t_removeClass(btnSubmit, 't-btn_sending'),
								(btnSubmit.tildaSendingStatus = '0')
							;(a = (l = t_forms__getErrorContainers(form, '')).errorBoxes),
								(n = l.allError)
							var l,
								d = xhr.responseText
							Array.prototype.forEach.call(n, function (t) {
								d
									? (t.innerHTML =
											d + '. Please, try again later. [' + xhr.status + ']')
									: xhr.statusText
									? (t.innerHTML =
											'Error [' +
											xhr.statusText +
			...
			if (
				...
				if (o.querySelector('.g-recaptcha') && grecaptcha) {
					...
					if (m) grecaptcha.reset(m)
						...
						;(m = grecaptcha.render(a + 'recaptcha', p)),
							(o.tildaCaptchaClientId = m)
					}
